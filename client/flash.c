...
// Get the state of the device, backwards compatible
static int get_device_state(uint32_t *state)
{
	UsbCommand c;
	...
	switch (resp.cmd) {
		...
		case CMD_DEVICE_INFO:
			*state = resp.arg[0];
			break;
		default:
			fprintf(stderr, "Error: Couldn't get device state, bad response type: 0x%04x\n", resp.cmd);
			return -1;
			break;
	}
