...
usb_dev_handle* findDevice(int verbose, unsigned int *iface)
{
  struct usb_bus *busses, *bus;
  usb_dev_handle *handle = NULL;
  struct device_unit units[50];
  int iUnit = 0;
  ...
  for (bus = busses; bus; bus = bus->next) {
    ...
    for (dev = bus->devices; dev; dev = dev->next) {
      ...
      if ((desc->idProduct == 0x0673) && (desc->idVendor == 0x16d0)) {
        handle = usb_open(dev);
        if (!handle) {
          if (verbose)
        ...
        *iface = dev->config[0].interface[0].altsetting[0].bInterfaceNumber;
        ...
        struct device_unit unit = {handle, {0}};
...
usb_dev_handle* OpenDevice(int verbose)
{
  int ret;
  usb_dev_handle *handle = NULL;
  unsigned int iface;

  handle = findDevice(verbose, &iface);
  if (!handle)
    return NULL;
